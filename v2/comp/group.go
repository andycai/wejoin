...
func (g Group) IsOwner(uid int64) bool {
	for _, member := range g.MembersV {
		if member.Id == uid && member.Pos == enum.PositionGroupOwner {
			return true
		}
	}
	return false
...
func (g Group) IsManager(uid int64) bool {
	for _, member := range g.MembersV {
		if member.Id == uid && member.Pos == enum.PositionGroupManager {
			return true
		}
	}
	return false
...
func (g Group) ManagerCount() int {
	c := 0
	for _, member := range g.MembersV {
		if member.Pos > enum.PositionGroupMember {
			c += 1
		}
	}
	return c
...
func (g *Group) Promote(uid int64) bool {
	for _, member := range g.MembersV {
		if member.Id == uid {
			member.Pos = enum.PositionGroupManager
			return true
		}
	}
	return false
...
func (g *Group) Transfer(uid, mid int64) bool {
	...
	for _, member := range g.MembersV {
		// 外部自行判断权限
		if member.Id == uid {
			member.Pos = enum.PositionGroupMember
		}
		if member.Id == mid {
			member.Pos = enum.PositionGroupOwner
			b = true
		}
	}
	return b
